...
std::string Hash::perceptual_hash(cv::Mat &img,int r,int c){
    ...
    cv::Mat dest3;
    cv::sort(dest, dest3, CV_SORT_ASCENDING);
    double median=dest3.at<double>(r/2,c-1);
    cv::Mat dest2(r,c,CV_8UC1);
    for(int i=0;i<r;i++){
        for(int j=0;j<c;j++){
    ...
    cv::imwrite("perceptual5.png", dest2);
    return res;
}
...
std::string Hash::wavelet_hash(cv::Mat &img,int r,int c){
    ...
    int median=dest3.at<uchar>(r/2,c-1);
    std::cout<<"median:"<<median<<std::endl;
    cv::Mat dest2(r,c,CV_8UC1);
    for(int i=0;i<r;i++){
        for(int j=0;j<c;j++){
            if(median <= dest.at<uchar>(i,j)){
                dest2.at<uchar>(i,j)=255;
                res+="1";
            }
            else{
